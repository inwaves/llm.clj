...
(defn encoder_forward
  ...
  (let [[B T _] @(t_size out)]
    (dotimes [b B]
      (dotimes [t T]
        (let [idx (first @(t_idx inp b t))
              wte_idx @(t_idx wte idx)
              wpe_t @(t_idx wpe t)]
          (swap! out update-in [b t] (fn [_] (mapv + wte_idx wpe_t))))))))
    ...
    (dotimes [b B]
      (dotimes [t T]
        (let [dout_bt @(t_idx dout b t)
              idx (first @(t_idx inp b t))]
          (dotimes [c C]
            (swap! dwte update-in [idx c]
                   (fn [dwte_idx_c] (+ (or dwte_idx_c 0) (nth dout_bt c))))
            (swap! dwpe update-in [t c]
                   (fn [dwpe_tc] (+ (or dwpe_tc 0) (nth dout_bt c))))))))))