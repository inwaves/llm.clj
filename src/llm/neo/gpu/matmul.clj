(ns llm.neo.gpu.matmul
  ...
  (:require [uncomplicate.commons.core :refer [with-release]]
            [uncomplicate.neanderthal.core :as ncore :refer [mm! axpy! trans mrows ncols zero!]]
...
(defn matmul-backward-gpu
  ...
  (let [bt (mrows inp-gpu)
        ...
        dbias ((resolve 'uncomplicate.neanderthal.cuda/cuv) oc)
        _ (zero! dbias)  ; Zero-initialize before accumulation
        _ (dotimes [i bt]
  ...
  (ncore/with-default-engine (gpu/cuda-engine)
    ...
    (with-release [inp-mat-cpu (neo/vec->matrix inp)
      ...
      (with-release [inp-gpu (gpu/to-gpu inp-mat-cpu)
                     ...
                     bias-gpu (when bias 
                               (gpu/to-gpu (native/dv (double-array bias))))]
